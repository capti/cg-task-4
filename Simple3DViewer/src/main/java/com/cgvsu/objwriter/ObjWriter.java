...
public class ObjWriter {
    ...
    private static void writeTextureVertices(BufferedWriter writer, List<Vector3f> textureVertices) throws IOException {
        DecimalFormatSymbols customSymbols = new DecimalFormatSymbols(Locale.US);
        customSymbols.setDecimalSeparator('.');
        DecimalFormat decimalFormat = new DecimalFormat("0.######", customSymbols);

        for (Vector3f textureVertex : textureVertices) {
            writer.write("vt " + decimalFormat.format(textureVertex.x) + " " + decimalFormat.format(textureVertex.y));
            writer.newLine();
        }
    }

    private static void writeNormals(BufferedWriter writer, List<Vector2f> normals) throws IOException {
        DecimalFormatSymbols customSymbols = new DecimalFormatSymbols(Locale.US);
        customSymbols.setDecimalSeparator('.');
        DecimalFormat decimalFormat = new DecimalFormat("0.######", customSymbols);

        for (Vector2f normal : normals) {
            writer.write("vn " + decimalFormat.format(normal.x) + " " + decimalFormat.format(normal.y) + " " + decimalFormat.format(normal));
            writer.newLine();
        }
    }
